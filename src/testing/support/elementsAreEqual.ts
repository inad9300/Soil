...
export function elementsAreEqual(e1: Element, e2: Element): void {
    assert.notStrictEqual(e1, e2, `trying to test if an element is equal to itself (tag <${e1.tagName}>)`)
    assert.isNotEmpty(e1.outerHTML, `the HTML of the created element is empty (tag <${e1.tagName}>)`)
    assert.equal(e1.outerHTML, e2.outerHTML, `the HTML of the two elements created is different (tag <${e1.tagName}>)`)
