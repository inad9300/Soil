    ...
    svgInterfaces
        .map(([tag, iface]) => {
            return voidElements.indexOf(tag) > -1
                ? `\texport const ${fixReservedTag(tag)} = (props?: BuiltTimeDom.${iface}): SvgTypesMap['${tag}'] => x('${tag}', props)`
                : `\texport const ${fixReservedTag(tag)} = (props?: BuiltTimeDom.${iface}, children?: ${elementsWithSpecialChildren[tag] || '(SVGElement | string)[]'}): SvgTypesMap['${tag}'] => x('${tag}', props, children)`
        })
        .join('\n')
}
}`)
